(() => {
  ...
  var MiBand5 = class extends MiBand {
    async init() {
      const device = await navigator.bluetooth.requestDevice({
        filters: [
          {
    ...
    async authenticate() {
      await this.startNotifications(this.chars.auth, async (e) => {
        ...
        const cmd = this.buf2hex(value.slice(0, 3));
        if (cmd === "100101") {
          console.log("Set new key OK");
        } else if (cmd === "100201") {
          const number = value.slice(3);
          console.log("Received authentication challenge: ", this.buf2hex(value.slice(3)));
          ...
          const cmd2 = this.concatBuffers(new Uint8Array([3, 0]), out);
